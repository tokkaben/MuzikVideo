...
async def activevc(_, message: Message):
    mystic = await message.reply_text(
        "Aktif Sesli Sohbetler Geliyorrr ❄️"
    )
    ...
    for x in served_chats:
        ...
        j += 1
    if not text:
        await mystic.edit_text("Aktif Sesli Sohbet Yok ❄️")
    else:
...
async def activevi_(_, message: Message):
    mystic = await message.reply_text(
        "Aktif Videolu Sohbetler Geliyorrr ❄️"
    )
    ...
    for x in served_chats:
        ...
        j += 1
    if not text:
        await mystic.edit_text("Aktif Videolu Sohbet Yok ❄️")
    else:
        await mystic.edit_text(
